...
static int socket_read(struct inode *inode, void *buf,
...
{
    int fd;
    struct socket_inode *inode;
    struct file *file;

    for (fd = 0; fd < OPEN_MAX; fd++)
        if (current_task->fd[fd].file == NULL)
            break;
    if (fd == OPEN_MAX)
        return -EMFILE; /* Too many open files */

    inode = socket_inode_create(family, type, protocol);
    if (!inode)
        return -1;
